...
def get_target_statement_opp(words):
    req = ''
    for sentence in words:
        if 'X' in sentence or 'x' in sentence or 'pieces' in sentence:
            nums = get_nums(sentence)
...
for f in sorted(os.listdir(my_path)):
    ...
    ind = result.index("ParsedText")+13
    ind2 = ind+result[ind:].index('","E')
    words = result[ind:ind2].split('\\n')

    req = get_target_statement_opp(words)
    if req:
        nums = get_nums(req)
        print(make_statement(nums))
    else: print(f)
...
image_path = my_path + 'IMG-20230708-WA0045.jpg'
result = ocr_space_file(image_path, api_key)
img = Image.open(image_path)
img
