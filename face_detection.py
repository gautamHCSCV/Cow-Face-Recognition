...
config = dict(
    saved_path="/content/drive/MyDrive/Cow Data/Saved_models/efficientnet_b4.pt",
    lr=0.001,
    EPOCHS = 5,
    BATCH_SIZE = 16,
    IMAGE_SIZE = 224,
...
#!unzip "/content/drive/MyDrive/Cattle Data/cow_data_final_7th_march23.zip"
...
# for i in images:
#     if len(set(dataset['labels']))==50 and i[1]!=dataset['labels'][-1]: break
#     dataset['images'].append(i[0])
#     dataset['labels'].append(i[1])


# import pickle
# a_file = open("/content/drive/MyDrive/Cow Data/Images/dataset_50.pkl", "wb")
# pickle.dump(dataset, a_file)
# a_file.close()

import pickle
a_file = open("/content/drive/MyDrive/Cow Data/Images/dataset_50.pkl", "rb")
...
#model.load_state_dict(torch.load('/content/drive/MyDrive/Cow Data/Saved_models/efficient.pt'))

train(model,train_loader,criterion, epochs = 15)
...
def evaluate(threshold = 2.5):
    ...
    for i in range(25):
        ...
        for j in range(i+1, min(len(images), i+5)):
